...
public class MsgConsumer {
    public static void main(String[] args) throws InterruptedException {
        ...
        // 消费分组名
        prop.put(ConsumerConfig.GROUP_ID_CONFIG, "firstGroup");
        ...
        while (true) {
            ...
            for (ConsumerRecord<String, String> record : records) {
                System.out.println("consumer1收到消息：topic:" + record.topic() + "|partition:" + record.partition()
                        + "|offeset:" + record.offset() + "|key:" + record.key() + "|value:" + record.value());
            }
            if (records.count() > 0) {
                // 提交offset
                consumer.commitSync();
        ...
        *//*consumer.assign(Arrays.asList(new TopicPartition(topicName, 1)));
        consumer.seek(new TopicPartition(topicName, 1), 70);*//*
        while (true){
            *//*
         ...
         *//*
            ConsumerRecords<String,String> records = consumer.poll(Integer.MAX_VALUE);
            for (ConsumerRecord<String,String> record : records) {
                System.out.println("consumer1收到消息：topic"+record.topic()+"|partition:"+record.partition()+"|offeset:"+record.offset()+"|key:"+record.key()+"|value:"+record
                .value());
            }
            if(records.count()>0){
                // 提交offset
